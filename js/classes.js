...
class Player extends Entity {
  constructor() {
    ...
    this.complete = false; // Tracking if level is complete
...
// player stopped moving and is at a game level ending event
    if (!this.moving && !this.complete) {
      this.checkSurvival(); // Check if level complete
    }
  }
...
// Draw the player on screen
  render() {
    ...
    this.moving = false; //  entity is moving
  }
...
// Reset the player to start position
  reset() {
    ...
    this.complete = false; // Reset levels completed data
  }

// Checks the players position to see if they complete the level
  checkSurvival() {
    this.y === 0 ? player.survival() : ''; // If at water(top of the board) survival, otherwise, nothing
  }

// Events when player hits the grass on the other side of path
  survival() {
    this.complete = true; // boolean set
  }
...
// Only decrement when greater than zero
    if (this.lives > 0) {
      ...
      lives[this.lives].classList.toggle('hide'); // change class to remove one life heart on screen
    }
  }
...
// modal message input and player life reset
  gameOver() {
    this.lives = 0; // player lost 3 hearts so lives = 0
    const lives = document.querySelectorAll('.lifebar img'); // create variable for the life meter
